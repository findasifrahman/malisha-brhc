<template>
  <section class="space-y-4">
    <DataTableShell title="Patient Stories" subtitle="Create and manage stories">
      <template #actions>
        <BaseButton
          size="sm"
      ...
      <div class="p-4">
        <div class="grid grid-cols-1 gap-3 md:grid-cols-12 md:items-end">
          <div class="md:col-span-8">
            ...
            <div class="mt-2"><BaseInput v-model="q" placeholder="Search stories..." /></div>
          </div>
        ...
        <div class="mt-4 overflow-x-auto">
          <table class="w-full text-left text-base">
            ...
            <tbody>
              ...
              <tr v-else-if="items.length === 0"><td class="py-6 text-brhc-muted" colspan="3">No stories found.</td></tr>
              <tr v-for="s in items" :key="s.id" class="border-b border-white/5">
                <td class="py-3 pr-4 font-semibold">{{ s.title }}</td>
                <td class="py-3 pr-4 text-brhc-muted">{{ s.isFeatured ? 'Yes' : 'No' }}</td>
                <td class="py-3 pr-4">
                  <div class="flex flex-wrap gap-2">
                    <BaseButton size="sm" variant="outline" @click="openEdit(s)">Edit</BaseButton>
                    <BaseButton size="sm" variant="outline" @click="remove(s)">Delete</BaseButton>
                  </div>
                </td>
              </tr>
            </tbody>
    ...
    <Modal v-model:open="modalOpen" :title="editing ? 'Edit Story' : 'Add Story'">
      <div class="space-y-4">
        <div>
        ...
        <div class="rounded-2xl ring-1 ring-black/10 p-4">
          <div class="flex items-center justify-between gap-3">
            <div>
              <div class="text-sm font-semibold">Media</div>
              <div class="text-sm text-brhc-muted">Attach images and/or videos from Media</div>
            </div>
            <BaseButton size="sm" variant="outline" @click="openMediaPicker">Attach</BaseButton>
          </div>

          <div v-if="selectedMedia.length === 0" class="mt-3 text-sm text-brhc-muted">No media attached.</div>
          <div v-else class="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4">
            <div v-for="m in selectedMedia" :key="m.id" class="rounded-2xl bg-white ring-1 ring-black/10 overflow-hidden">
              ...
              />
              <div v-else class="h-24 w-full bg-gray-100 flex items-center justify-center text-xs text-gray-600">
    ...
    <Modal v-model:open="mediaPickerOpen" title="Select media" subtitle="Choose images and/or videos">
      <div class="space-y-4">
        <div>
...
async function load() {
  loading.value = true
  try {
    const res = await adminList<any>(auth.token, '/admin/patientstory', { page: page.value, pageSize: 20, q: q.value || undefined })
...
async function openEdit(s: any) {
  ...
  editing.value = s
  try {
    const res = await adminGet<any>(auth.token, `/admin/patientstory/${s.id}`)
    const full = res.data
    editing.value = full
...
async function save() {
  ...
  try {
    ...
    if (editing.value?.id) {
      await adminPut(auth.token, `/admin/patientstory/${editing.value.id}`, payload)
      id = editing.value.id
    } else {
      const created = await adminPost<any>(auth.token, '/admin/patientstory', payload)
      id = created.data.id
    }

    await adminPost(auth.token, `/admin/patientstory/${id}/media`, { mediaIds: Array.from(selectedMediaIds.value) })
...
async function remove(s: any) {
  ...
  try {
    await adminDelete(auth.token, `/admin/patientstory/${s.id}`)
    await load()
  } catch (e) {
    toast.push({ title: 'Error', message: e instanceof Error ? e.message : String(e), variant: 'error' })
  }
