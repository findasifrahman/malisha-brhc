<template>
  <section class="space-y-4">
    <DataTableShell title="Services" subtitle="Create and manage services">
      <template #actions>
        <BaseButton
          size="sm"
      ...
      <div class="p-4">
        <div class="grid grid-cols-1 gap-3 md:grid-cols-12 md:items-end">
          <div class="md:col-span-8">
            ...
            <div class="mt-2"><BaseInput v-model="q" placeholder="Search services..." /></div>
          </div>
        ...
        <div class="mt-4 overflow-x-auto">
          <table class="w-full text-left text-base">
            ...
            <tbody>
              ...
              <tr v-else-if="items.length === 0"><td class="py-6 text-brhc-muted" colspan="4">No services found.</td></tr>
              <tr v-for="s in items" :key="s.id" class="border-b border-black/5">
                <td class="py-3 pr-4 font-semibold">{{ s.name }}</td>
                <td class="py-3 pr-4 text-brhc-muted">{{ s.serviceType }}</td>
    ...
    <Modal v-model:open="modalOpen" :title="editing ? 'Edit Service' : 'Add Service'">
      <div class="space-y-4">
        <div>
        ...
        <div>
          <div class="text-sm font-semibold text-brhc-muted">Service Type</div>
          <div class="mt-2"><BaseInput v-model="form.serviceType" placeholder="e.g. International Support" /></div>
        </div>
...
async function load() {
  loading.value = true
  try {
    const res = await adminList<any>(auth.token, '/admin/services', { page: page.value, pageSize: 20, q: q.value || undefined })
...
async function openEdit(s: any) {
  ...
  editing.value = s
  try {
    const res = await adminGet<any>(auth.token, `/admin/services/${s.id}`)
    const full = res.data
    editing.value = full
    form.name = full.name ?? ''
    form.slug = full.slug ?? ''
    form.serviceType = full.serviceType ?? ''
...
async function save() {
  if (!form.name.trim() || !form.serviceType.trim()) {
    toast.push({ title: 'Validation', message: 'Name and serviceType are required.', variant: 'info' })
    return
  }
  ...
  try {
    let id: string
    if (editing.value?.id) {
      await adminPut(auth.token, `/admin/services/${editing.value.id}`, {
        name: form.name,
      ...
      const created = await adminPost<any>(auth.token, '/admin/services', {
        name: form.name,
        slug: form.slug || undefined,
        serviceType: form.serviceType,
        isFeatured: form.isFeatured,
    ...
    await adminPost(auth.token, `/admin/services/${id}/media`, { mediaIds: Array.from(selectedMediaIds.value) })
...
async function remove(s: any) {
  ...
  try {
    await adminDelete(auth.token, `/admin/services/${s.id}`)
    await load()
  } catch (e) {
    toast.push({ title: 'Error', message: e instanceof Error ? e.message : String(e), variant: 'error' })
  }
