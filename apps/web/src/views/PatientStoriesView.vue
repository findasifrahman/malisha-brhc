  ...
  <section class="py-8">
    <div class="mx-auto w-full max-w-7xl space-y-10 px-4 lg:px-6">
      <div class="rounded-3xl border border-red-100 bg-gradient-to-br from-white via-red-50/40 to-yellow-50/30 p-6 shadow-sm">
        <SectionHeader title="Patient Stories" subtitle="Experiences and journeys." />
        ...
        <div class="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center">
          <div class="flex-1 rounded-2xl border border-red-100 bg-white px-3 py-2 shadow-sm">
            <div class="flex items-center gap-2">
              ...
              <BaseInput v-model="q" placeholder="Search stories..." class="w-full border-0 bg-transparent px-0 focus:outline-none" />
            </div>
          </div>
        ...
        <RouterLink v-for="s in items" :key="s.id" :to="`/patient-stories/${s.slug}`" class="group">
          ...
          <div class="relative h-44 overflow-hidden">
            ...
            <div v-else class="flex h-full w-full items-center justify-center bg-gradient-to-br from-red-50 to-yellow-50">
              <div class="text-sm font-semibold text-red-600">Story</div>
            </div>
          ...
          <div class="relative p-6 flex-1 flex flex-col justify-between">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                ...
                <div v-if="s.excerpt" class="mt-1 text-sm text-gray-600 line-clamp-2">{{ s.excerpt }}</div>
              </div>
            ...
            <div class="mt-4 grid grid-cols-1 gap-2 text-sm">
              <div v-for="(sd, idx) in (s.shortDetails ?? []).slice(0, 2)" :key="idx" class="flex items-start gap-2">
...
import { MessageSquareText, Search, Sparkles } from 'lucide-vue-next'
...
async function load() {
  loading.value = true
  try {
    const res = await listPatientStories({ page: page.value, pageSize: 10, q: q.value || undefined })
