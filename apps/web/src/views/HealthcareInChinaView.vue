  ...
  <section class="py-8">
    <div class="mx-auto w-full max-w-7xl space-y-10 px-4 lg:px-6">
      <div class="rounded-3xl border border-red-100 bg-gradient-to-br from-white via-red-50/40 to-yellow-50/30 p-6 shadow-sm">
        <SectionHeader title="Advanced Healthcare in China" subtitle="Guides, topics, and advanced programs." />
        ...
        <div class="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center">
          <div class="flex-1 rounded-2xl border border-red-100 bg-white px-3 py-2 shadow-sm">
            <div class="flex items-center gap-2">
              ...
              <BaseInput v-model="q" placeholder="Search topics..." class="w-full border-0 bg-transparent px-0 focus:outline-none" />
            </div>
          </div>
    ...
    <div v-else-if="items.length === 0" class="rounded-2xl border border-red-100 bg-white p-6 text-sm text-brhc-muted">
      ...
      <div v-else class="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-3">
        <RouterLink v-for="a in items" :key="a.id" :to="`/healthcare-in-china/${a.slug}`" class="group">
          ...
          <div class="relative p-6">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                ...
                <div class="mt-1 text-sm text-gray-600 line-clamp-2">{{ a.description || 'Explore the latest treatment insights and innovations.' }}</div>
              </div>
            ...
            <div class="mt-4 grid grid-cols-1 gap-2 text-sm">
              <div v-for="(d, idx) in (a.shortDetails ?? []).slice(0, 2)" :key="idx" class="flex items-start gap-2">
...
import { Lightbulb, Search, Sparkles } from 'lucide-vue-next'
...
async function load() {
  loading.value = true
  try {
    const res = await listHealthcareInChina({ page: page.value, pageSize: 12, q: q.value || undefined })
    const detailed = await Promise.all(
      (res.items ?? []).map(async (a: any) => {
        try {
